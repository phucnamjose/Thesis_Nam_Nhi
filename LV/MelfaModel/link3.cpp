...
Link3::Link3() :// Class Link3 la con cua class OpenGLWindow (OpenGLWindow la class tu dinh nghia)
    m_indexBuffer(QOpenGLBuffer::IndexBuffer)
  , m_filepath("MelfaModel/models/Part3.STL")
...
void Link3::createShaderProgram(QString vShader, QString fShader)
{
...
void Link3::createAttributes()  // Attribute: thuoc tinh
{
    if(m_error)
        return;
...
void Link3::setupLightingAndMatrices()
{
    ...
    m_lightInfo.Position = QVector4D( -1.0f, 1.0f, 1.0f, 1.0f );// vi tri
    m_lightInfo.Intensity = QVector3D( 1.0f, 1.0f, 1.0f);       // cuong do

// Gray color
//    m_materialInfo.Ambient = QVector3D( 0.625f, 0.625f, 0.625f );   // mau xung quanh QVector3D( 0.05f, 0.2f, 0.05f );
//    m_materialInfo.Diffuse = QVector3D( 0.3f, 0.3f, 0.3f );   // mau khuech tan QVector3D( 0.3f, 0.5f, 0.3f );
...
void Link3::my_paintGL(int w_xRot, int w_yRot, int w_zRot, float w_scale, QMatrix4x4 w_projection, double degJ1, double degJ2)
{
    if(m_error)
        return;
    ...
    // Calculate DH matrix
    //double d1 = 0.3f, a2 = 0.25f;
    double L2 = 0.16f,L1 = 0.197f,alpha = 0.0f,d1 = 0.2275;
...
void Link3::drawNode(const Node *node, QMatrix4x4 objectMatrix)
{
    // Prepare matrices
...
void Link3::setMaterialUniforms(MaterialInfo &mater)
{
