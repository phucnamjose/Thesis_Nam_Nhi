...
Box::Box() :// Class Box la con cua class OpenGLWindow (OpenGLWindow la class tu dinh nghia)
    m_indexBuffer(QOpenGLBuffer::IndexBuffer)
  , m_filepath("MelfaModel/models/Box.STL")
...
void Box::createShaderProgram(QString vShader, QString fShader)   // do^? bo'ng
{
...
void Box::setupLightingAndMatrices()
{
...
void Box::my_paintGL(int w_xRot, int w_yRot, int w_zRot, float w_scale, QMatrix4x4 w_projection)
{
    if(m_error)
        return;
...
void Box::drawNode(const Node *node, QMatrix4x4 objectMatrix)
{
    // Prepare matrices
...
void Box::setMaterialUniforms(MaterialInfo &mater)
{
