...
Link6::Link6() :// Class Link6 la con cua class OpenGLWindow (OpenGLWindow la class tu dinh nghia)
    m_indexBuffer(QOpenGLBuffer::IndexBuffer)
  , m_filepath("MelfaModel/models/MITSU - Link6-1.STL")
...
void Link6::createShaderProgram(QString vShader, QString fShader)   // do^? bo'ng
{
...
void Link6::setupLightingAndMatrices()
{
    ...
    m_materialInfo.Ambient = QVector3D( 0.3f, 0.05f, 0.15f );
    m_materialInfo.Diffuse = QVector3D( 0.5f, 0.3f, 0.4f );
    m_materialInfo.Specular = QVector3D( 0.6f, 0.6f, 0.6f );
    m_materialInfo.Shininess = 50.0f;
}

void Link6::my_paintGL(int w_xRot, int w_yRot, int w_zRot, float w_scale, QMatrix4x4 w_projection, double degJ1, double degJ2, double degJ3, double degJ4, double degJ5)
{
    if(m_error)
        return;
    ...
    double c5 = cos(degJ5), s5 = sin(degJ5);
    m_DH = QMatrix4x4( c1*c5*c234 - s1*s5, - c5*s1 - c1*c234*s5, c1*s234, c1*(a2*s2 + a3*s23) + c1*d5*s234,
                       c1*s5 + c5*c234*s1,   c1*c5 - c234*s1*s5, s1*s234, s1*(a2*s2 + a3*s23) + d5*s1*s234,
                                 -c5*s234,              s5*s234,    c234,    d1 + a2*c2 + a3*c23 + c234*d5,
                                        0,                    0,       0,                                1);
...
void Link6::drawNode(const Node *node, QMatrix4x4 objectMatrix)
{
    // Prepare matrices
...
void Link6::setMaterialUniforms(MaterialInfo &mater)
{
